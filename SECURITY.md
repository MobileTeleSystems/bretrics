...
2. Send a detailed report to the repository maintainer via:
   - GitHub Security Advisory: [Report a vulnerability](https://github.com/MobileTeleSystems/bretrics/security/advisories/new)
   - Email: Create an issue in the [issue tracker](https://github.com/MobileTeleSystems/bretrics/issues) marked as **Security** (if no sensitive details need to be shared)
...
As a Prometheus metrics microservice, `bretrics`:

- **Accepts user input** - all metrics input is validated and sanitized
- **Does not store data** - metrics are exposed via Prometheus exporter endpoint
...
- Open a [GitHub Security Advisory](https://github.com/MobileTeleSystems/bretrics/security/advisories/new)
- Create an issue at: <https://github.com/MobileTeleSystems/bretrics/issues>
